...
<table width="213" cellspacing="0" cellpadding="0" border="0">
	...
	<%
		List<WatchInfo> wis = duty.getCityDuty(today);
		request.setAttribute("city_t", wis);
	%>

	<c:forEach items="${city_t}" var="duty">
	<tr>
		<td width="80" align="center">${duty.watchdept }：</td>
		<td width="130">${duty.watchmen } ${duty.watchtel }</td>
	</tr>
	</c:forEach>
	
	<c:if test="${fn:length(city_t) <=0 }">
		<tr>
			<td width="100%" align="center" colspan="2" >值班信息尚未录入</td>
		</tr>
	</c:if>
</table>
<br />
